...
if "user_spec" in globals():
    cluster, cluster_spec, cluster_error = cf.clusterize(deepmd_iterative_apath,training_iterative_apath,step="freezing",user_keyword=user_spec)
else:
    cluster, cluster_spec, cluster_error = cf.clusterize(deepmd_iterative_apath,training_iterative_apath,step="freezing")
if cluster_error != 0:
...
cf.check_file(jobs_apath/("job_deepmd_freeze_"+cluster_spec["arch_type"]+"_"+cluster+".sh"),True,True,"No SLURM file present for the freezing step on this cluster.")
slurm_file_master = cf.read_file(jobs_apath/("job_deepmd_freeze_"+cluster_spec["arch_type"]+"_"+cluster+".sh"))
del jobs_apath
...
for it_nnp in range(1, config_json["nb_nnp"] + 1):
    ...
    cf.write_file(local_apath/("job_deepmd_freeze_"+cluster_spec["arch_type"]+"_"+cluster+".sh"),slurm_file)
