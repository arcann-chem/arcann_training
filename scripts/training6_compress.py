...
cf.check_file(deepmd_iterative_path+'/jobs/training/job_deepmd_compress_'+arch_type+'_'+cluster+'.sh',0,0)
slurm_file_master = cf.read_file(deepmd_iterative_path+'/jobs/training/job_deepmd_compress_'+arch_type+'_'+cluster+'.sh')
...
for it_nnp in range(1, config_json['nb_nnp'] + 1):
    ...
    cf.check_file('job_deepmd_compress_'+arch_type+'_'+cluster+'.sh',0,1,'Training NNP '+str(it_nnp)+' was not lauched. No job file found.')
...
del it_nnp, config_json, config_json_fpath, deepmd_model_version, deepmd_iterative_path, slurm_email, slurm_file, slurm_file_master, allocation_name, project_name, training_json, training_json_fpath
del cluster, training_iterative_apath, current_iteration, current_iteration_zfill, arch_name, arch_type
