...
if count != (len( exploration_json['subsys_nr']) * exploration_json['nb_nnp'] * exploration_json['nb_traj'] ):
    logging.critical('Some jobs failed or are still running. Check manually.')
    logging.critical('Aborting...')
    sys.exit(1)
else:
    ...
    for it_subsys_nr in exploration_json['subsys_nr']:
        ...
        del it_nnp
    del it_subsys_nr
del count, exploration_json, exploration_json_fpath

logging.info('Exploration success')
