...
def main(
    ...
    if curr_iter > 0:
        ...
        previous_testing_json = load_json_file(
            (control_path / f"testing_{padded_prev_iter}.json")
        )
    else:
    ...
    if not user_input_json_present and "deepmd_model_version" not in user_input_json:
        current_input_json["deepmd_model_version"] = training_json[
            "deepmd_model_version"
        ]
    ...
    user_machine_keyword = get_machine_keyword(
        current_input_json, previous_testing_json, default_input_json, "test"
    )
    ...
    current_input_json["job_email"] = get_key_in_dict(
        "job_email", user_input_json, previous_testing_json, default_input_json
    )
    ...
    if current_input_json["is_compressed"]:
        ...
        nnp_list = [
            f"graph_{nnp}_{padded_curr_iter}.pb"
            for nnp in range(1, main_json["nnp_count"] + 1)
        ]
    ...
    for idx_nnp, nnp in enumerate(nnp_list):
        ...
        job_file = replace_substring_in_string_list(
            job_file, "_R_DEEPMD_MODEL_FILE_", f"{nnp}"
        )
