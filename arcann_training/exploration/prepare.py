...
def main(
    ...
    current_input_json = generate_input_exploration_json(
        user_input_json,
        previous_exploration_json,
        default_input_json,
        current_input_json,
    ...
    job_array_params_file["sander_emle"] = [
        "PATH/_R_DEEPMD_VERSION_/_R_MODEL_FILES_/_R_SANDER_IN_FILE_/_R_TOP_FILE_/_R_COORD_FILE_/_R_EMLE_YAML_FILE_/_R_EMLE_MODEL_FILE_//_R_PLUMED_FILES_/"
    ]
    ...
    for system_auto_index, system_auto in enumerate(main_json["systems_auto"]):
        ...
        # SANDER-EMLE
            ...
            master_system_sander_emle_in = textfile_to_string_list(
                training_path / "user_files" / (system_auto + ".in")
            )
        ...
        if system_exploration_type == "lammps":
            ...
            if curr_iter == 1:
                ...
                current_input_json["job_walltime_h"][system_auto_index] = (
                    system_walltime_approx_s / 3600
                )
                current_input_json["exp_time_ps"][system_auto_index] = (
        ...
        # SANDER-EMLE
            ...
            if curr_iter == 1:
                ...
                system_sander_emle_data_path = (
                    training_path / "user_files" / system_sander_emle_data_fn
                )
                ...
                # Get the cell and nb of atoms: It can be done now because the starting point is the same by NNP and by traj
                system_nb_atm, num_atom_types, box, masses, coords = read_lammps_data(
                    training_path / "user_files" / (system_auto + ".lmp")
                )
                system_cell = [box[1] - box[0], box[3] - box[2], box[5] - box[4]]
        ...
        # i-PI // UNTESTED
            ...
            # Subsequent ones
                ...
                current_input_json["system_exp_time_ps"] = (
                    system_nb_steps * system_timestep_ps
                )
        ...
        for nnp_index in range(1, main_json["nnp_count"] + 1):
            for traj_index in range(1, system_traj_count + 1):
                ...
                if system_exploration_type == "lammps":
                    ...
                    input_replace_dict["_R_SEED_THER_"] = (
                        f"{nnp_index}{random.randrange(0, 1000)}{traj_index}{padded_curr_iter}"
                    )
                    input_replace_dict["_R_DCD_OUT_"] = (
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}.dcd"
                    )
                    input_replace_dict["_R_RESTART_OUT_"] = (
                    ...
                    # Plumed files
                    if plumed[0]:
                        input_replace_dict["_R_PLUMED_IN_"] = (
                            f"plumed_{system_auto}.dat"
                        )
                        input_replace_dict["_R_PLUMED_OUT_"] = (
                    ...
                    # Write DATA file
                    string_list_to_textfile(
                        local_path / system_lammps_data_fn,
                        system_lammps_data,
                        read_only=True,
                    )
                    ...
                    job_file = replace_substring_in_string_list(
                        job_file,
                        "_R_LAMMPS_LOG_FILE_",
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}.log",
                    )
                    job_file = replace_substring_in_string_list(
                        job_file,
                        "_R_LAMMPS_OUT_FILE_",
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}.out",
                    )
                    job_file = replace_substring_in_string_list(
                ...
                # SANDER-EMLE
                    ...
                    input_replace_dict["_R_NC_OUT_"] = (
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}.nc"
                    )
                    input_replace_dict["_R_RESTART_OUT_"] = (
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}.restrt"
                    )
                    input_replace_dict["_R_DEVI_OUT_"] = (
                        f"model_devi_{system_auto}_{nnp_index}_{padded_curr_iter}.out"
                    )
                    ...
                    if curr_iter > 1:
                        ...
                        input_replace_dict["_R_COORD_FILE_"] = (
                            system_sander_emle_data_fn
                        )
                    ...
                    system_emle_model_file = (
                        training_path / "user_files" / f"{system_auto}.mat"
                    )
                    system_prmtop_file = (
                        training_path / "user_files" / f"{system_auto}.prmtop"
                    )

                    subprocess.run(
                        ["cp", f"{system_emle_model_file}", local_path],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                    )
                    subprocess.run(
                        ["cp", f"{system_prmtop_file}", local_path],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                    )
                    subprocess.run(
                        ["cp", f"{system_sander_emle_data_path}", local_path],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT,
                    )
                    ...
                    #  Write INPUT file
                    for key, value in input_replace_dict.items():
                        system_sander_emle_in = replace_substring_in_string_list(
                            system_sander_emle_in, key, value
                        )
                    del key, value
                    string_list_to_textfile(
                        local_path / f"{system_auto}_{nnp_index}_{padded_curr_iter}.in",
                    ...
                    system_sander_emle_yaml["deepmd_deviation"] = (
                        f"model_devi_{system_auto}_{nnp_index}_{padded_curr_iter}.out"
                    )
                    system_sander_emle_yaml["energy_file"] = (
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}_emle.en"
                    )
                    system_sander_emle_yaml["log_file"] = (
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}_emle.log"
                    )
                    system_sander_emle_yaml["model"] = f"{system_auto}.mat"
                    system_sander_emle_yaml["qm_xyz_file"] = (
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}_QM.xyz"
                    )
                    system_sander_emle_yaml["qm_xyz_frequency"] = int(
                        system_print_every_x_steps
                    )

                    with (
                        local_path
                        / f"{system_auto}_{nnp_index}_{padded_curr_iter}.yaml"
                    ).open("w") as f:
                    ...
                    job_file = replace_substring_in_string_list(
                        job_file,
                        "_R_SANDER_RESTART_FILE_",
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}.ncrst",
                    )
                    job_file = replace_substring_in_string_list(
                        job_file,
                        "_R_EMLE_OUT_FILE_",
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}_emle.out",
                    )
                    job_file = replace_substring_in_string_list(
                        job_file,
                        "_R_SANDER_TRAJOUT_FILE_",
                        f"{system_auto}_{nnp_index}_{padded_curr_iter}.nc",
                    )
                    job_file = replace_substring_in_string_list(
                        job_file, "_R_TOP_FILE_", f"{system_auto}.prmtop"
                    )
                    job_file = replace_substring_in_string_list(
                        job_file,
                        "_R_SANDER_COORD_FILE_",
                        f"{system_sander_emle_data_fn}",
                    )
                    job_file = replace_substring_in_string_list(
                ...
                # i-PI
                    ...
                    input_replace_dict["_R_SEED_"] = (
                        f"{nnp_index}{random.randrange(0, 1000)}{traj_index}{padded_curr_iter}"
                    )
                    input_replace_dict["_R_SYS_"] = (
                    ...
                    job_file = replace_substring_in_string_list(
                        job_file, "_R_MODEL_FILES_", f"{models_list[0]}"
                    )
                    job_file = replace_substring_in_string_list(
                        job_file,
                        "_R_IPI_IN_FILE_",
