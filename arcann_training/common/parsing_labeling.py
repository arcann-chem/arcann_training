...
def extract_and_convert_energy(energy_in, energy_out, system_candidates_not_skipped_counter, conversion_factor=1.0, program=None, version=None):
    if program == "cp2k":
        ...
        for line in energy_in:
            ...
            if match:
                ...
                energy_out[system_candidates_not_skipped_counter - 1] = energy_array[0] * conversion_factor
        return energy_out
    elif program == "orca":
...
# TODO: Add tests for this function
@catch_errors_decorator
def extract_and_convert_coordinates(coordinates_in, coordinates_out, system_candidates_not_skipped_counter, conversion_factor=1.0):
    ...
    coordinates_out[system_candidates_not_skipped_counter - 1, :] = coord_array * conversion_factor
    return coordinates_out


# TODO: Add tests for this function
@catch_errors_decorator
def extract_and_convert_box_volume(input, box_out, volume_out, system_candidates_not_skipped_counter, conversion_factor=1.0, program=None, version=None):
    if program == "cp2k":
...
# TODO: Add tests for this function
@catch_errors_decorator
def extract_and_convert_forces(forces_in, forces_out, system_candidates_not_skipped_counter, conversion_factor=1.0, program=None, version=None):
    if program == "cp2k":
        del forces_in[0:4]
        del forces_in[-1]
        ...
        forces_out[system_candidates_not_skipped_counter - 1, :] = forces_array * conversion_factor
        return forces_out
    elif program == "orca":
    ...
    if program == "cp2k":
        if version < 8 and version >= 6:
            ...
            if matching_index is not None:
                ...
                virial_out[system_candidates_not_skipped_counter - 1, :] = stress_xyz_array * volume[system_candidates_not_skipped_counter - 1] / conversion_factor
...
# TODO: Add tests for this function
@catch_errors_decorator
def extract_and_convert_wannier(wannier_in, wannier_out, system_candidates_not_skipped_counter, nb_atom, conversion_factor=1.0, program=None, version=None):
    if program == "cp2k":
        ...
        wannier_out[system_candidates_not_skipped_counter - 1, :] = wannier_array * conversion_factor
