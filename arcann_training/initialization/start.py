...
from arcann_training.initialization.utils import generate_main_json, check_properties_file, check_dptrain_properties, check_lmp_properties, check_typeraw_properties
...
def main(
    ...
    if "systems_auto" not in user_input_json:
        ...
        arcann_logger.info(f"Auto-populated 'systems_auto' with: {user_input_json['systems_auto']}")
        ...
        for system_auto in user_input_json["systems_auto"]:
            if not (user_files_path / f"{system_auto}.lmp").is_file():
                arcann_logger.error(f"File not found: {user_files_path / f'{system_auto}.lmp'} but requested as system")
                arcann_logger.error(f"Aborting...")
                return 1
        arcann_logger.info(f"Using 'systems_auto' from the input JSON: {user_input_json['systems_auto']}")
    else:
