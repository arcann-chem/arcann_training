...
from deepmd_iterative.common.filesystem import remove_file
...
def main(
    ...
    for system_auto_index, system_auto in enumerate(labeling_json["systems_auto"]):
        logging.info(f"Processing system: {system_auto} ({system_auto_index + 1}/{len(labeling_json['systems_auto'])})")
        ...
        if system_candidates_skipped_count > 0 or system_disturbed_candidates_skipped_count > 0:
            ...
            logging.info(f"{system_auto}: {system_candidates_skipped_total_count} jobs skipped ({system_candidates_skipped_count}|{system_disturbed_candidates_skipped_count}). List in '{skipped_file}'.")
        ...
        logging.info(f"Processed system: {system_auto} ({system_auto_index + 1}/{len(labeling_json['systems_auto'])})")
