...
def main(
    ...
    for nnp in range(1, main_json["nnp_count"] + 1):
        ...
        if (local_path / f"job_deepmd_test_{machine_spec['arch_type']}_{machine}.sh").is_file():
            ...
            try:
                ...
                arcann_logger.critical(f"DP Test - '{nnp}' NOT launched - '{machine_launch_command}' not found.")
    ...
    if completed_count == main_json["nnp_count"]:
        ...
        arcann_logger.critical(f"Replace the key 'is_launched' to 'True' in the 'testing_{padded_curr_iter}.json'.")
    del completed_count
