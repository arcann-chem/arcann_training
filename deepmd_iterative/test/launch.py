...
def main(
    ...
    if not testing_json["is_prepared"]:
        logging.error(f"Lock found. Please execute 'test prepare' first.")
        logging.error(f"Aborting...")
        return 1
    ...
    for nnp in range(1, main_json["nnp_count"] + 1):
        ...
        if (local_path / f"job_deepmd_test_{machine_spec['arch_type']}_{machine}.sh").is_file():
            change_directory(local_path)
            try:
                subprocess.run([machine_launch_command, f"./job_deepmd_test_{machine_spec['arch_type']}_{machine}.sh"])
                logging.info(f"DP Test - '{nnp}' launched.")
                completed_count += 1
            except FileNotFoundError:
                logging.critical(f"DP Test - '{nnp}' NOT launched - '{machine_launch_command}' not found.")
            change_directory(local_path.parent)
        else:
            logging.critical(f"DP Test - '{nnp}' NOT launched - No job file.")
        del local_path
    del nnp
    ...
    if completed_count == main_json["nnp_count"]:
        ...
        logging.critical(f"Replace the key 'is_launched' to 'True' in the 'testing_{padded_curr_iter}.json'.")
    del completed_count
