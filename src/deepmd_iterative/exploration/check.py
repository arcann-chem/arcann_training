...
def main(
        ...
        for it_nnp in range(1, config_json["nb_nnp"] + 1 ):
            for it_number in range(1, exploration_json["nb_traj"] + 1):
                ...
                if exploration_json["exploration_type"] == "lammps":
                    lammps_output_file = local_path/f"{it_subsys_nr}_{it_nnp}_{current_iteration_zfill}.log"
                ...
                # ### i-PI
                    ...
                    ipi_output_file = local_path/f"{it_subsys_nr}_{it_nnp}_{current_iteration_zfill}.i-PI.server.log"
    ...
    if (completed_count + skipped_count + forced_count) != (len( exploration_json["subsys_nr"]) * exploration_json["nb_nnp"] * exploration_json["nb_traj"] ):
        logging.error(
