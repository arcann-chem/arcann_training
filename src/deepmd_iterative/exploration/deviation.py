    ...
    for it0_subsys_nr, it_subsys_nr in enumerate(config_json["subsys_nr"]):
        ...
        for it_nnp in range(1, config_json["nb_nnp"] + 1):
            for it_number in range(1, exploration_json["nb_traj"] + 1):
                ...
                model_deviation_filename = f"model_devi_{it_subsys_nr}_{it_nnp}_{current_iteration_zfill}.out"
                ...
                nb_steps_expected = ( exploration_json["subsys_nr"][it_subsys_nr]["nb_steps"] // exploration_json["subsys_nr"][it_subsys_nr]["print_every_x_steps"] ) + 1 - start_row_number
                ...
                if not (local_path / "skip").is_file():
                    ...
                    if nb_steps_expected > ( total_row_number - start_row_number ):
                        ...
                        logging.critical(f"Exploration {it_subsys_nr}/{it_nnp}/{it_number}")
                        logging.critical(f"Mismatch between expected ({nb_steps_expected}) number of steps")
                        logging.critical(f"and actual ({total_row_number}) number of steps in the deviation file.")
                    ...
                    # This part is when sigma_high_limit was crossed (Gets stats before)
                        ...
                        std_deviation_max_f = np.std(model_deviation[start_row_number:end_row_number, 4])
                    ...
                    if ( QbC_stats["nb_good"] + QbC_stats["nb_rejected"] + QbC_stats["nb_candidates"] ) < nb_steps_expected:
                        QbC_stats["nb_rejected"] = QbC_stats["nb_rejected"] + QbC_stats - ( QbC_stats["nb_good"] + QbC_stats["nb_rejected"] + QbC_stats["nb_candidates"] )
                ...
                exploration_json["subsys_nr"][it_subsys_nr]["nb_total"] = exploration_json["subsys_nr"][it_subsys_nr]["nb_total"] + QbC_stats["nb_total"]
                exploration_json["subsys_nr"][it_subsys_nr]["nb_candidates"] = exploration_json["subsys_nr"][it_subsys_nr]["nb_candidates"] + QbC_stats["nb_candidates"]
        ...
        exploration_json["subsys_nr"][it_subsys_nr]["mean_deviation_max_f"] = exploration_json["subsys_nr"][it_subsys_nr]["mean_deviation_max_f"] / ( exploration_json["nb_nnp"] +  len(range(1, exploration_json["nb_traj"] + 1)) - skipped_traj )
        exploration_json["subsys_nr"][it_subsys_nr]["std_deviation_max_f"] = exploration_json["subsys_nr"][it_subsys_nr]["std_deviation_max_f"] / ( exploration_json["nb_nnp"] +  len(range(1, exploration_json["nb_traj"] + 1)) - skipped_traj )
        ...
        for it_nnp in range(1, config_json["nb_nnp"] + 1):
            for it_number in range(1, exploration_json["nb_traj"] + 1):
                ...
                if not (local_path / "skip").is_file():
                    ...
                    if exploration_json["subsys_nr"][it_subsys_nr]["nb_candidates"] <= max_candidates:
                        selection_factor = 1
                    else:
                        selection_factor = QbC_stats["nb_candidates"] / exploration_json["subsys_nr"][it_subsys_nr]["nb_candidates"]

                    # Get the local max_candidates
                    QbC_stats["selection_factor"] = selection_factor
                    max_candidates_local = int(np.floor(max_candidates*selection_factor))
                    ...
                    if len(candidate_indexes) > max_candidates_local:
                        ...
                        kept_indexes = np.concatenate(([candidate_indexes[0]], kept_indexes, [candidate_indexes[-1]]))
                    ...
                    if kept_indexes.shape[0] > 0:
                        ...
                        for kept_idx in kept_indexes:
                            temp_min = model_deviation[:,4][np.where(model_deviation[:,0] == kept_idx)]
                            if temp_min < min_val:
                                min_val = temp_min
                                min_index = kept_idx
                ...
                exploration_json["subsys_nr"][it_subsys_nr]["nb_kept"] = exploration_json["subsys_nr"][it_subsys_nr]["nb_kept"] + QbC_stats["nb_kept"]
