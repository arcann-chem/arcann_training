...
def generate_starting_points(exploration_type: int,
                             it_subsys_nr: int,
                             training_path: str,
                             previous_iteration_zfill: str,
                             prevexploration_json: dict,
                             input_present: bool,
                             subsys_disturbed_start: bool) -> Tuple[List[str], List[str], bool]:
    """
    ...
    starting_points_path = list(Path(training_path, "starting_structures").glob(f"{previous_iteration_zfill}_{it_subsys_nr}_*.{file_extension}"))
    ...
    if input_present and subsys_disturbed_start:
        ...
        if ( prevexploration_json["subsys_nr"][it_subsys_nr]["disturbed_start"]
...
def create_models_list(config_json: dict, prevtraining_json: dict, it_nnp: int, previous_iteration_zfill: str, training_path: Path, local_path: Path) -> Tuple[List[str], str]:
    """
    ...
    for it_sub_nnp in range(1, config_json["nb_nnp"] + 1 ):
        nnp_apath = (training_path/"NNP"/("graph_"+str(it_sub_nnp)+"_"+previous_iteration_zfill+compress_str+".pb")).resolve()
...
def update_nb_steps_factor(prevexploration_json: dict, it_subsys_nr: int) -> int:
    ...
    ratio_ill_described = (
        (prevexploration_json["subsys_nr"][it_subsys_nr]["nb_candidates"] + prevexploration_json["subsys_nr"][it_subsys_nr]["nb_rejected"]) / prevexploration_json["subsys_nr"][it_subsys_nr]["nb_total"]
    )
