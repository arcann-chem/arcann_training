...
for dataset in data/*/ ; do
    if [[ -d "${dataset%/}" ]]; then
        ...
        # With details
        #${SRUN_DeepMD_EXE} test -m ${DeepMD_MODEL_FILE} -s "${dataset%/}" -d "${dataset_name}" -n 100000000 > "${dataset_name}.out" 2>&1
        # Without details
        ${SRUN_DeepMD_EXE} test -m ${DeepMD_MODEL_FILE} -s "${dataset%/}" -n 100000000 > "${dataset_name}.out" 2>&1
