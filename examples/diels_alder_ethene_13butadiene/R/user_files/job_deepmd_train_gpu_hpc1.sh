...
cp "${DeepMD_IN_FILE}" "${TEMPWORKDIR}" && echo "${DeepMD_IN_FILE} copied successfully"
[ -d ${DeepMD_DATA_DIR} ] || { echo "${DeepMD_DATA_DIR} does not exist. Aborting..."; exit 1; }
mkdir -p "${TEMPWORKDIR}/data" || { echo "Could not create ${TEMPWORKDIR}/data. Aborting..."; exit 1; }
{ cp -r ${DeepMD_DATA_DIR}/* "${TEMPWORKDIR}/data" && echo "${DeepMD_DATA_DIR} copied successfully"; } || { echo "Could not copy ${DeepMD_DATA_DIR}. Aborting..."; exit 1; }
...
${SRUN_DeepMD_EXE} train ${DeepMD_IN_FILE} --log-path ${DeepMD_LOG_FILE} > ${DeepMD_OUT_FILE} 2>&1
